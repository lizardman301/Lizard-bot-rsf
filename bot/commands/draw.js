...
module.exports = {
	...
	async execute(interaction) {
		...
		let data, games;
		try {
			const rsGame = game !== null ? game : 'sfv';
		...
		const player2 = interaction.options.getMember('player');
		if (player1.id === player2.id) {
			...
			for (let i = 0; i < NUM_CHARS; i++) {
				const rsChar = data[Math.floor(Math.random() * data.length)];
			...
			const playerOrder = [[player1, player2, player1, player2, player2, player1], [player2, player1, player2, player1, player1, player2]][Math.floor(Math.random() * 2)];
			let currTurn = 0;
